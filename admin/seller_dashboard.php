...
if (!isset($_SESSION['sell_step'])) $_SESSION['sell_step'] = 1;
if (!isset($_SESSION['sell_data']) || !is_array($_SESSION['sell_data'])) $_SESSION['sell_data'] = [];
        ...
        switch ($step) {
            case 1: // Produce
                if ($input === '' || !in_array($input, $market_connect_produce_list, true)) {
                    ...
                    $_SESSION['sell_data'] = [];
                    $_SESSION['sell_data']['produce_name'] = $input;
            ...
            case 2: // Location
                if ($input === '') {
                    ...
                    $_SESSION['sell_data']['location'] = $input;
                    $_SESSION['sell_step'] = 3;
                }
                break;

            case 3: // Quantity unit
                if ($input === "1") {
                    $_SESSION['sell_data']['quantity_unit'] = "KG";
                    $_SESSION['sell_step'] = 4;
                } elseif ($input === "2") {
                    $_SESSION['sell_data']['quantity_unit'] = "50kg Bag";
                    $_SESSION['sell_step'] = 4;
                } else {
            ...
            case 4: // Quantity value
                if (!is_numeric($input) || $input <= 0) {
                    ...
                    $_SESSION['sell_data']['quantity_value'] = $input;
                    $_SESSION['sell_step'] = 5;
                }
                break;

            case 5: // Price
                $unit = $_SESSION['sell_data']['quantity_unit'] ?? 'KG';
                ...
                if (!is_numeric($input) || $input < $range['floor'] || $input > $range['ceiling']) {
                    ...
                    $_SESSION['sell_data']['price'] = $input;
                    $_SESSION['sell_step'] = 6;
                }
                break;

            case 6: // Confirm
                if ($input === "1") {
                    $p = $_SESSION['sell_data'];
                    ...
                    try {
                        ...
                        $stmt_insert = $db_conn->prepare("
                            INSERT INTO produce_sell (User_ID, Produce_ID, Location, Quantity_Value, Quantity_Unit, Price)
                            VALUES (:uid, :pid, :loc, :qv, :qu, :price)
                        ");
                        $stmt_insert->execute([
                            'uid'   => $user_id,
                            'pid'   => $produce_id,
                        ...
                        // Wrapped so a problem here never blocks the actual posting.
                        try {
                            $msg = "Hi {$user_name}, your listing for {$p['quantity_value']} {$p['quantity_unit']} of "
                                 . "{$p['produce_name']} at MWK {$p['price']} was posted.";
                            $alert_id = queueAlert($db_conn, $user_id, 'listing_posted', $msg);
                            sendAlert($db_conn, $alert_id);
                        } catch (PDOException $alertEx) {
                            // ignore for now - alerts are still a scaffold
                        }

                        logActivity($db_conn, $user_id, 'post_sell_listing',
                            "{$p['quantity_value']} {$p['quantity_unit']} {$p['produce_name']} at MWK {$p['price']}");

                        $_SESSION['sell_step'] = 7;
...
<head>
    <meta charset="utf-8">
    <title>Seller Dashboard - Market Connect</title>
...
<div class="container mt-3">
    ...
    <div class="card p-3 shadow-sm">
        <h3 class="text-success mb-2">🌾 Seller Dashboard</h3>
        ...
        <form method="POST">
            ...
            <?php elseif ($step == 4): ?>
                <label>Enter quantity (in <?= htmlspecialchars($_SESSION['sell_data']['quantity_unit']) ?>)</label>
            ...
            <?php elseif ($step == 5):
                ...
                $range = $price_ranges[$unit];
            ?>
                <label>Enter price per <?= htmlspecialchars($unit) ?></label>
            ...
            <?php elseif ($step == 6): ?>
                <h5>Confirm posting?</h5>
                <p class="mb-1">1. ✅ Post &amp; Submit</p>
                <p>2. ❌ Cancel</p>
                <input type="text" name="value" class="form-control" required>

            <?php elseif ($step == 7): ?>
                <h5>✅ Posting Successful!</h5>
                <p class="mb-1">1. Add another produce</p>
